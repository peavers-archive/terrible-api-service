...
/**
 * History objects contain a list of MediaFile objects. Each time a new object is added to History we look for a History
 * object that fits within the current month. If one is found we append to the list of MediaFiles; if none is found we
 * create a new History object with the year-month ID pairing.
 */
@Slf4j
@Service
...
public class HistoryServiceImpl implements HistoryService {
    ...
    public Mono<History> getHistory() {
        ...
        return repository.findById(getHistoryId()).map(history -> {
    ...
    /**
     * Create an ID with a year-month pattern. This is not expected to be unique for each History object as we either
     * want to create a new object each month, or update the current object.
     */
